...
router.post("/users/create/:signup_code", async (req, res) => {
  ...
  try {
    ...
    const user = new User({
      ...
      dob: signup.dob,
      is_client: signup.is_client,
      is_employee: signup.is_employee
    });
    // const user = new User({
    //   email: signup.email,
    //   password: signup.password,
    ...
    if(userSaved){
      ...
      if(signup.is_client){//Si es cliente
      ...
      } else if(signup.is_employee){ //Si es empleado
        ...
        const employee = new Employee({
          ...
          second_lastname: signup.second_lastname,
          position_id: "0"
          // position_id: mongoose.Types.ObjectId(signup.position_id)
        })

        await employee.save().then(()=>{
          console.log('Employed created...')
        })
        .catch(async() =>{
          await User.findOneAndDelete({ _id: userSaved._id })
          console.log("User Deleted");
...
router.patch("/users/me", auth, async (req, res) => {
  ...
  console.log(req.user.is_client)
  console.log(req.user.is_employee)
  ...
  try {
    ...
    //Actualizar si es cliente
    if(req.user.is_client){

    }
    //Actualizar si es empleado
    if(req.user.is_employee){
...
router.post("/users/login", async (req, res) => {
  ...
  try {
    ...
    if(user.is_client){
      ...
      return res.status(200).send({ user: client, token });
    }

    if(user.is_employee){
      const employee = await Employee.findOne({user_id: user._id})
      return res.status(200).send({ user: employee, token });
    }
